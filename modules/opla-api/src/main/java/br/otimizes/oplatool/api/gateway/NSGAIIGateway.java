...
@Component
public class NSGAIIGateway implements IGateway<NSGAIIConfigs> {

    public NSGAIIGateway(Persistence persistence, EdCalculation edCalculation) {
    ...
    private static final Logger LOGGER = Logger.getLogger(NSGAIIGateway.class);
    ...
    public void execute(NSGAIIConfigs experimentCommonConfigs) throws Exception {
        ...
        for (String xmiFilePath : plas) {
            ...
            Experiment experiment = persistence.save(plaName, "NSGAII", experimentCommonConfigs.getDescription(),
                    OPLAThreadScope.hashOnPosteriori.get());
            ExperimentConfigurations conf = new ExperimentConfigurations(experiment.getId(), "NSGAII",
            ...
            if (experimentCommonConfigs.isLog())
                logInformation(xmiFilePath, experimentCommonConfigs, experimentCommonConfigs.getPopulationSize(),
                        experimentCommonConfigs.getMaxEvaluations(),
                        experimentCommonConfigs.getCrossoverProbability(),
                        experimentCommonConfigs.getMutationProbability());

            List<String> selectedObjectiveFunctions = experimentCommonConfigs.getOplaConfigs()
                    .getSelectedObjectiveFunctions();
            ...
            for (int runs = 0; runs < experimentCommonConfigs.getNumberOfRuns(); runs++) {
                ...
                Execution execution = AlgorithmBaseUtils.getExecution(experimentCommonConfigs, result, experiment,
            ...
            AlgorithmBaseUtils.logAndSave(experimentCommonConfigs, plaName, result, experiment, allRuns,
                    selectedObjectiveFunctions, persistence, edCalculation);
    ...
    private Algorithm getAlgorithm(OPLA problem, NSGAIIConfigs experimentCommonConfigs) throws JMException {
        ...
        Crossover crossover = CrossoverFactory.getCrossoverOperator("PLACrossoverOperator", parametersCrossover,
                experimentCommonConfigs);
        ...
        Mutation mutation = MutationFactory.getMutationOperator("PLAMutationOperator", parametersMutation,
                experimentCommonConfigs);
