...
//        response.setHeader("Access-Control-Allow-Headers", "Content-Type, authorization, Connection, group");
        ...
        if (request.getRequestURI().contains("/api/llm") || request.getRequestURI().equals("/api/user/login") || request.getRequestURI().equals("/api/user/forgot") || request.getMethod().equals("OPTIONS")) filterChain.doFilter(request, response);
