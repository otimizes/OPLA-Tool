...
public class UntilBestOPLABase {
    ...
    public UntilBestOPLABase(Persistence persistence, EdCalculation edCalculation) {
    ...
    public void execute(NSGAIIConfigs configs) throws Exception {
        ...
        for (String pla : plas) {
            ...
            Experiment experiement = persistence.save(plaName, "UntilBest", configs.getDescription(), OPLAThreadScope.hash.get());

            Algorithm algorithm;
            Result result = new Result();
            experiment = persistence.save(plaName, "NSGAII", configs.getDescription(), OPLAThreadScope.hash.get());
            ExperimentConfigurations conf = new ExperimentConfigurations(experiement.getId(), "UntilBest", configs);
