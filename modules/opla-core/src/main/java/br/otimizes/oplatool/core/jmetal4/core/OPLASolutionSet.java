...
//        return Arrays.stream(solutionSet.writeObjectivesToMatrix()).map(p -> Arrays.asList(ArrayUtils.toObject(p))
...
//        List<List<Element>> collect = solutionSet.getSolutionSet().stream().map(s -> s.getAlternativeArchitecture()
