...
import br.otimizes.oplatool.core.jmetal4.util.Permutation;
...
public class Niching {
    ...
    public void associate() {
        ...
        for (int k = 0; k < mgPopulation.size(); k++) {
            ...
            sol.setClusterIDForMetaHeuristics(index);
    ...
    public void assignment() {
        ...
        for (int k = 0; k < population.size(); k++) {
            ro[population.get(k).getClusterIDForMetaHeuristics()]++;
        }
        ...
        while (num < remain) {
            ...
            for (int k = 0; k < lastFront.size(); k++) {
                if (lastFront.get(k).getClusterIDForMetaHeuristics() == id)
                    list.add(k);
            }
