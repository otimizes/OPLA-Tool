...
public class OPLA extends Problem {
    ...
    public SolutionSet removeDominadas(SolutionSet result) {
        List<Solution> collect = result.getSolutionSet().stream().filter(r ->  (r.getEvaluation() >= 5 && r.getEvaluatedByUser())  || (r.containsArchitecturalEvaluation() && r.getEvaluatedByUser())).collect(Collectors.toList());
