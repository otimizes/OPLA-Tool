...
public class GenerateArchitecture extends ArchitectureBase {
    ...
    private static Set<Method> createMethods(Element klass) {
        ...
        Set<br.otimizes.oplatool.architecture.representation.Method> methodsClass = new HashSet<>();
        ...
        for (br.otimizes.oplatool.architecture.representation.Method method : methodsClass) {
            ...
            if (method.isAbstract()) {
                br.otimizes.oplatool.architecture.papyrus.touml.Method m = br.otimizes.oplatool.architecture.papyrus.touml.Method.create().withId(method.getId())
    ...
    private static List<br.otimizes.oplatool.architecture.papyrus.touml.Attribute> createAttributes(Operations op, Class klass) {
        List<br.otimizes.oplatool.architecture.papyrus.touml.Attribute> attributes = new ArrayList<br.otimizes.oplatool.architecture.papyrus.touml.Attribute>();

        for (Attribute attribute : klass.getAllAttributes()) {
            br.otimizes.oplatool.architecture.papyrus.touml.Attribute attr = br.otimizes.oplatool.architecture.papyrus.touml.Attribute.create().withName(attribute.getName())
    ...
    public void generate(Architecture a, String output) {
        ...
        try {
            ...
            for (Class klass : a.getAllClasses().stream().sorted(Comparator.comparing(Element::getNamespace)).collect(Collectors.toList())) {
                ...
                List<br.otimizes.oplatool.architecture.papyrus.touml.Attribute> attributesForClass = createAttributes(op, klass);
