...
public class GenerateArchitecture extends ArchitectureBase {
    ...
    private static Set<Method> createMethods(Element klass) {
        ...
        for (br.ufpr.dinf.gres.architecture.representation.Method method : methodsClass) {
            ...
            if (method.isAbstract()) {
                br.ufpr.dinf.gres.architecture.touml.Method m = br.ufpr.dinf.gres.architecture.touml.Method.create().withId(method.getId())
    ...
    private static List<br.ufpr.dinf.gres.architecture.touml.Attribute> createAttributes(Operations op, Class klass) {
        List<br.ufpr.dinf.gres.architecture.touml.Attribute> attributes = new ArrayList<br.ufpr.dinf.gres.architecture.touml.Attribute>();

        for (Attribute attribute : klass.getAllAttributes()) {
            br.ufpr.dinf.gres.architecture.touml.Attribute attr = br.ufpr.dinf.gres.architecture.touml.Attribute.create().withName(attribute.getName())
    ...
    public void generate(Architecture a, String output) {
        ...
        try {
            ...
            for (Class klass : a.getAllClasses().stream().sorted(Comparator.comparing(Element::getNamespace)).collect(Collectors.toList())) {
                ...
                List<br.ufpr.dinf.gres.architecture.touml.Attribute> attributesForClass = createAttributes(op, klass);
                ...
                for (br.ufpr.dinf.gres.architecture.touml.Attribute attr : attributesForClass) {
